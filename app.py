...
def get_data(stock_id):
    ...
    # 🎯 關鍵修復：不再用 dropna 殺光資料，改用 fillna 補齊，保護新上市 ETF
    ...
    df['RSI'] = df['RSI'].fillna(50) # 新股預設 RSI 給 50
...
def calculate_factors_and_score(df, chips, live_price, prev_close):
    ...
    if live_price >= L['Resist']: bulls["價量與型態"].append(f"突破近 21 日壓力線 ({L['Resist']:.1f})")
    if live_price <= L['Support']: bears["價量與型態"].append(f"跌破近 21 日支撐線 ({L['Support']:.1f})")
    ...
    if bonus2_surge: bulls["價量與型態"].append("🔥 飆股共振：MA7/14/21 雙重金叉")
    if bonus1_safe: bulls["價量與型態"].append("中期多頭：均線站穩 MA35 之上")
...
# --- 🚀 超薄頂部導覽列 ---
col_logo, col_input, col_btn, col_hist = st.columns([1.2, 1, 0.5, 6], gap="small")
...
with st.spinner(f"分析 ({tc}) 中..."):
    ...
    if df is not None:
        ...
        st.error(f"⚠️ 找不到股票代碼 **{tc}** 的資料。可能已下市或輸入錯誤。").m-val { font-size:16px; font-weight:900; color:#fff; line-height:1.2; }
.text-red { color:#f85149 !important; } .text-green { color:#3fb950 !important; } .text-white { color:#fff !important; } .text-yellow { color:#d29922 !important; }
details > summary { list-style: none; outline: none; cursor: pointer; padding: 2px; transition: 0.2s; display: flex; align-items: center; color: #c9d1d9; font-size: 12px;}
details > summary::-webkit-details-marker { display: none; }
details > summary:hover { color: #fff; }
