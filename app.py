    ...
    try:
        ...
        if not divs.empty:
            ...
            if ttm_div > 0 and live_price > 0:
                calculated_yield = float(ttm_div / live_price)
    except:
        pass

    # 如果手動算得出來，就強制覆寫 info 裡面的殖利率
    if calculated_yield is not None:
        info['dividendYield'] = calculated_yield
...
def generate_ai_summary(total_score, chg_pct):
    if chg_pct <= -3.0: return "🥶 <strong style='color:#3fb950;'>恐慌破底</strong>：單日帶量長黑，動能惡化，切勿摸底接刀。"
    if chg_pct >= 4.0: return "🚀 <strong style='color:#f85149;'>強勢點火</strong>：長紅大漲表態，短線動能極強。"
    if total_score >= 8:
        return "🔥 <strong style='color:#f85149;'>主升段啟動</strong>：共振指標達成，籌碼技術皆優。"
    elif total_score >= 5:
        return "📈 <strong style='color:#f85149;'>穩健偏多</strong>：基礎多方成立，沿均線防守。"
    elif total_score <= 2:
        return "🧊 <strong style='color:#3fb950;'>弱勢空頭</strong>：跌破關鍵均線且大戶資金流出。"
    else:
        return "⚖️ <strong style='color:#c9d1d9;'>區間震盪</strong>：多空勢均力敵，方向未明朗。"
...
with col_logo: st.markdown(
    "<div style='color:#58a6ff; font-weight:900; font-size:15px; margin-top:8px;'>⚡ 股票決策終端</div>",
    unsafe_allow_html=True)
with col_input: st.text_input("代碼", value=st.session_state['target'], key="search_input",
...
with st.spinner(f"分析 ({tc}) 中..."):
    ...
    if df is not None:
        ...
        with col_main:
            ...
            yoy_val = f"<span class='{'text-red' if rev['yoy'] > 0 else 'text-green'}'>{rev['yoy']:.1f}%</span>" if rev[
                                                                                                                        'status'] == 'success' else "-"
