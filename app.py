...
/* 🚀 終極修復：強制導覽列字體為絕對白色，並提升背景對比 */
.nav-pills { display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 1px solid #30363d; padding-bottom: 10px; }
div[data-testid="stRadio"] > div { display: flex; gap: 15px; background-color: #161b22; padding: 6px; border-radius: 8px; border: 1px solid #30363d; width: fit-content; }
div[data-testid="stRadio"] label { cursor: pointer; padding: 8px 20px; border-radius: 6px; font-weight: bold; transition: 0.2s; } 
/* 無論選中與否，字體強制純白 */
div[data-testid="stRadio"] label * { color: #ffffff !important; font-size: 15px !important; }
div[data-testid="stRadio"] label:hover { background-color: rgba(255,255,255,0.1); }
/* 隱藏預設圓圈 */
div[data-testid="stRadio"] input { display: none; }
/* 選中時的高亮底色 */
div[data-testid="stRadio"] [aria-checked="true"] { background-color: #1f3a5f !important; border: 1px solid #58a6ff; }
/* 選中時的字體顏色 (亮藍色) */
div[data-testid="stRadio"] [aria-checked="true"] * { color: #58a6ff !important; }
...
SECTOR_MAP = {
    "半導體": ["2330", "2454", "2303", "3711", "3443", "3661"],
    "AI 伺服器": ["2382", "3231", "2356", "2376", "2324"],
    "AI 散熱": ["3324", "3017", "2421", "3653", "3483"],
    "矽光子 (CPO)": ["3363", "3163", "4979", "3450"],
    "重電綠能": ["1519", "1513", "1514", "1504", "1609"],
    "電腦週邊 (PC/NB)": ["2353", "2357", "2377", "2395"],
    "網通與光通訊": ["2345", "3596", "5388", "4906"],
    "PCB 族群": ["2368", "3037", "8046", "3044", "2313"],
    "貨櫃航運": ["2603", "2609", "2615", "2606"],
    "金融保險": ["2881", "2882", "2891", "2886", "2884"],
    "營建資產": ["2504", "2520", "2542", "2548", "5522"],
    "生技醫療": ["1795", "6472", "4743", "6446", "1707"]
}
...
def get_market_data():
    ...
    elif live_price < L['MA20']: status, suggest, msg = "跌破月線", "現金 50%~70%", "大盤轉弱跌破關鍵均線，建議嚴控風險。"
    ...
    market_info = {"price": live_price, "chg": chg, "chg_pct": chg_pct, "ma20": L['MA20'], "rsi": L['RSI'], "status": status, "suggest": suggest, "msg": msg}
    ...
    if not df_t86.empty:
        ...
        sector_flow = [{"sector": s, "foreign": 0, "trust": 0, "total": 0, "tooltip": "系統連線錯誤"} for s in SECTOR_MAP.keys()]
...
def render_market_dashboard():
    ...
    for f in flow:
        ...
        bg = "transparent"
        if f['total'] >= 10000: bg = "rgba(248,81,73,0.25)" 
        elif f['total'] >= 3000: bg = "rgba(248,81,73,0.1)" 
        elif f['total'] <= -10000: bg = "rgba(63,185,80,0.25)" 
        elif f['total'] <= -3000: bg = "rgba(63,185,80,0.1)" 
...
def calculate_factors_and_score(df, chips, live_price, prev_close):
    ...
    if L['OBV'] > L['OBV_MA14']: bulls["技術指標"].append("OBV > 14日線 (買盤強)")
    else: bears["技術指標"].append("OBV < 14日線 (資金流出)")
